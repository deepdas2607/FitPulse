...
export const analyzeExerciseForm = (
  ...
  switch (exerciseId) {
    ...
    case 'jumping-squats': {
      ...
      const DOWN_THRESHOLD = 150;    // Angle below this = consider "down" position (Super relaxed)
      const UP_THRESHOLD = 160;       // Angle above this = consider "up" position (Standard standing)
      // Note: If UP is 160 and DOWN is 150, we have 10 degree hysteresis.
    ...
    case 'hammer-curl': {
      ...
      const CURL_DOWN_THRESHOLD = 110;  // Arm curled - Relaxed from 100
      const CURL_UP_THRESHOLD = 110;    // Arm extended - Relaxed from 120 (Note: slight overlap handled by state)
      // Actually, let's keep a gap to avoid jitter:
      // Down < 105, Up > 115
      const CURL_DOWN_STRICT = 105;
      const CURL_UP_STRICT = 115;
      ...
      switch (exerciseState.repState) {
        case 'UP':
          ...
          if (avgElbowAngle < 105) { // CURL_DOWN_STRICT
        ...
        case 'DOWN':
          ...
          if (avgElbowAngle > 115 && timeSinceLastRep > CURL_MIN_TIME) { // CURL_UP_STRICT
            exerciseState.repState = 'UP';
            exerciseState.repCount++;
            exerciseState.lastRepTime = currentTime;
