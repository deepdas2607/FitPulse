...
const ExerciseSummary: React.FC<ExerciseSummaryProps> = ({ session, onClose, onStartNew }) => {
  ...
  const uniqueIssues = Array.from(new Set(session.formIssues.map(i => i.message)));
  const totalIssueFrames = session.formIssues.length;

  // Base score 100
  // Deduct 10 points for each UNIQUE issue type detected (e.g. "Keep back straight")
  // Deduct 0.1 points for each frame the issue persisted (frequency penalty)
  let formScore = 100;
  formScore -= uniqueIssues.length * 10;
  formScore -= totalIssueFrames * 0.1;

  formScore = Math.round(Math.max(0, Math.min(100, formScore)));
